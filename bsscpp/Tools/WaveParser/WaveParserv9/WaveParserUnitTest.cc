...
float testingFloatingReading(WaveParser& wp, int bufferSize, FILE* file2comparewith, int channel)
{
	...
	{
		sum += (samples[channel-1][i] - buffer[i]);
	}
...
int main(int argc, char* argv[])
{
	try
	{
		...
		for(int i=0; i<20; i++)
		{
			if(testingFloatingReading(wp, 100, arquivo,1) == 0.0)
		...
		if(testingFloatingReading(wp, 1192320, arquivo,1) == 0.0)
			cout << "Left Channel SUM: 0 - RESULT: OK!" << endl;
		else
			cout << "SOMETHING WENT WRONG!!!!" << endl;
		...
		arquivo = NULL;
		if( (arquivo = fopen( "felipeRightFloat.dat", "rb" )) == NULL )
			cout << "The file was not opened" << endl;
		else
			cout << "The file was opened" << endl;
		...
		if(testingFloatingReading(wp, 1192320, arquivo,2) == 0.0)
			cout << "Right Channel SUM: 0 - RESULT: OK!" << endl;
		else
			cout << "SOMETHING WENT WRONG!!!!" << endl;
