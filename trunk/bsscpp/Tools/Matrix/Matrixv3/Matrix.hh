...
class Matrix
{
	...
	public:
		...
		template <class D>
		friend Matrix<D> inv(const Matrix<D>& op2);
		template <class D> 
...
class Eye : public Matrix<T>
{
	public:
		...
		//template <class D> 
		//friend Eye<D> inv(const Matrix<D>& eye);
};
...
/*template <class D>
Eye<D> inv(const Matrix<D>& eye)
{	
	if(typeid(eye) != typeid(Eye<D>))
		throw "Matrix must be of type Eye!";
...
template <class D>
Matrix<D> inv(const Matrix<D>& op2)  
{
	if(op2.getNumberOfColumns() != op2.getNumberOfRows())
		throw "Matrix is not square!!!";
