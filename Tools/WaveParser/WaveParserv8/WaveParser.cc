...
WaveParser::WaveParser(const string& fileName)
{
	...
	if(this->openWaveFile(fileName))
	{
		if(this->findID("RIFF") && this->findID("WAVE"))
		{
			...
			{
				this->sampleRateM = WAV.sampleRate;
				this->numChannelsM = WAV.numChannels;
				this->bitsPerSampleM = WAV.bitsPerSample;
				this->dataSubChunkSizeM = DINFO.dataSubChunkSize;
				this->offsetFromBeginningM = DINFO.offsetFromBeginning;
				...
				this->totalSamplesM = this->dataSubChunkSizeM/(this->numChannelsM * (this->bitsPerSampleM/8));
				this->readSamplesM = 0;
...
bool WaveParser::goBackToBeginOfDataChunk()
{
	...
	if(this->readSamplesM == this->totalSamplesM)
		this->readSamplesM = 0;
}

void WaveParser::rewind()
{
	this->readSamplesM = 0;
}
...
int WaveParser::readWave(float** samples, int nOfSamples2Read)
{
	...
	{
		samples[0][i] = (float) (((buffer[4*i+1] << 8) | (((int)buffer[4*i]) & 0xFF))/32768.0);
		samples[1][i] = (float) (((buffer[4*i+3] << 8) | (((int)buffer[4*i+2]) & 0xFF))/32768.0);
	}
...
void WaveParser::printWaveFileInfo()
{
	...
	cout << "Total number of Samples: " << this->totalSamplesM << endl;

	cout << "Number of read Samples so far: " << this->readSamplesM << endl;

	cout << "Left Channel Current Position: " << this->leftChannelCurrentPosM << endl;
	if(this->numChannelsM == 2)
		cout << "Right Channel Current Position: " << this->rightChannelCurrentPosM << endl;
}
