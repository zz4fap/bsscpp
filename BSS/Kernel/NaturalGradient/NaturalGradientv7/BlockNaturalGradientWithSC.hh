...
class BlockNaturalGradientWithSC
{
	...
	protected:
		...
		UINT getWsc(Matrix<T>& R1, Matrix<T>& R2, VectorMatrix *WmAux, VectorMatrix*& Wsc);
};
...
void BlockNaturalGradientWithSC<T>::getNaturalGradient(T** y, UINT numOfSamples, const VectorMatrix* Wm, VectorMatrix*& deltaW, double& costFunctionValue, Matrix<T>*& bdiagRyy, Matrix<T>*& Ryy, Matrix<T>*& outputPowerOverIterations)
{
	...
	for(int epoch = 0; epoch < numOfEpochs; epoch++)
	{
		...
		//--------------------------------------------------------------//
	    	ccm->getCrossCorrelationMatrix(y1,y2,(*R[1])); // R12						// Point of problem with speed....... (OK)
	    	acm->getOutputPowerMatrix(y2, (T)10.0, (*R[3])); // R22

	    	if(det((*R[3])) != 0) //If the determinant of R is equal to zero this test fails.		// Point of problem with speed.......
		{
			Rtemp = (*R[1])*inv((*R[3])); 								// Point of problem with speed....... (OK)
		...
		//--------------------------------------------------------------//
	    	acm->getOutputPowerMatrix(y1, (T)10.0, (*R[0])); // R11
	    	(*R[2]) = transpose((*R[1])); // R21

	    	if(det((*R[0])) != 0) //If the determinant of R is equal to zero this test fails.		// Point of problem with speed.......
		{
			Rtemp = (*R[2])*inv((*R[0]));								// Point of problem with speed.......	
...
template <class T>
UINT BlockNaturalGradientWithSC<T>::getWsc(Matrix<T>& R1, Matrix<T>& R2, VectorMatrix *WmAux, VectorMatrix*& Wsc)
{
	Matrix<T> Rtemp(D,D);
